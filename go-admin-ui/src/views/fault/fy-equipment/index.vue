...
使用:inline="true"属性指定表单项为行内布局，以便在页面上占用更小的空间。 -->
        ...
        >
          ...
          <el-form-item label="光纤" prop="fabric">
            <el-select
              ...
              @keyup.enter.native="handleQuery"
              ><el-option
                v-for="item in fabricOptionsForm"
                :key="item.id"
                :label="item.fabric"
                :value="item.fabric"
              />
          ...
          <el-form-item label="光栅" prop="raster">
            <el-select
              ...
              @keyup.enter.native="handleQuery"
              ><el-option
                v-for="(raster, index) in rasterlist"
                :key="index"
                :label="raster"
                :value="raster"
              />
        ...
        <el-dialog :title="title" :visible.sync="open" width="500px">
          <el-form ref="form" :model="form" :rules="rules" label-width="80px">
            <el-form-item label="设备名" prop="equipname">
              <el-input v-model="form.equipname" placeholder="设备名(唯一)" />
            </el-form-item>
            <el-form-item label="设备编码" prop="equipcode">
              <el-input v-model="form.equipcode" placeholder="设备编码(唯一)" />
            </el-form-item>
            <el-form-item label="负责人" prop="username">
            ...
            <el-form-item label="光纤" prop="fabric">
              <!-- <el-input v-model="form.fabric" placeholder="光纤" /> -->
              <el-select
                v-model="form.fabric"
                placeholder="光纤"
                clearable
                filterable
                @change="validateMaxRaster"
                ><el-option
                  v-for="item in fabricOptionsForm"
                  :key="item.id"
                  :label="item.fabric"
            ...
            <el-form-item label="光栅" prop="raster">
              <!-- <el-input v-model="form.raster" placeholder="光栅" /> -->
              <el-select
                v-model="form.raster"
                placeholder="光栅"
                allow-create
...
import { listFyFabric } from "@/api/admin/fy-fabric";
  ...
  data() {
    return {
      ...
      rules: {
        ...
        fabric: [{ required: true, message: "光纤不能为空", trigger: "blur" }],
        raster: [
          {
            required: true,
            validator: (rule, value, callback) => {
              if (Math.max(...value) > this.maxRaster) {
                callback(
                  new Error(`${this.form.fabric}的光栅数需 ≤${this.maxRaster}`)
                );
              } else {
                callback();
              }
      ...
      //其他数据
      rastercount: undefined, //光栅数量
      rasterlist: undefined, //光栅表
      maxRaster: undefined, //光纤对应光栅数量
    };
  },
  created() {
    this.getList();
  ...
  methods: {
    ...
    getList() {
      ...
      listFyFabric(
        ...
        this.fabricOptionsForm = response.data.list;
        this.rastercount = Math.max(
          ...this.fabricOptionsForm.map((item) => item.raster)
        );
        this.maxRaster = this.rastercount;
        let arr = [];
        for (let i = 1; i <= this.rastercount; i++) {
          arr.push(i);
        }
        this.rasterlist = arr;
      });
    ...
    handleQueryFull() {
      ...
      listFyEquipment(
        this.addDateRange({ pageSize: 10000 }, this.dateRange)
      ).then((response) => {
        this.fyEquipmentList = response.data.list;
        this.total = response.data.count;
      });
      //表单获取地区数据
      listFyArea(this.addDateRange({ pageSize: 10000 }, this.dateRange)).then(
        (response) => {
          this.areaOptionsForm = response.data.list;
        }
      );
      //表单获取状态数据
      listFyStatus(this.addDateRange({ pageSize: 10000 }, this.dateRange)).then(
        (response) => {
          this.statusOptionsForm = response.data.list;
        }
      );
      //表单获取型号数据
      listFyType(this.addDateRange({ pageSize: 10000 }, this.dateRange)).then(
        (response) => {
          this.typeOptionsForm = response.data.list;
        }
      );
      //表单获取负责人数据
      listFyUser(this.addDateRange({ pageSize: 10000 }, this.dateRange)).then(
        (response) => {
          this.userOptionsForm = response.data.list;
        }
      );
    ...
    handleUpdate(row) {
      ...
      getFyEquipment(id).then((response) => {
        ...
        //以tempraster作为媒介，对多选下拉框进行数据回显
        if (this.form.raster !== "" && this.form.raster !== undefined) {
          const tempraster = this.form.raster.split(",");
          this.form.raster = tempraster.map((raster) => parseInt(raster, 10));
        } else {
          this.form.raster = undefined;
        }

        this.open = true;
        this.title = "修改设备台账";
        // console.log("hhhhh",this.form.fabric);
        this.validateMaxRaster();
        this.isEdit = true;
      });
    },
    /** 提交按钮 */
    submitForm: function () {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          //faultcount处理
          if (!this.form.faultcount) {
            console.log(this.form.faultcount);
            this.form.faultcount = "0";
          }
          //raster数组处理
          if (this.form.raster !== ("" || undefined)) {
            this.form.raster = this.form.raster.join(",");
          }
    ...
    //表单效验，fabric对应raster最大值
    validateMaxRaster() {
      // console.log("调用了validateMaxRaster");
      const fabric = this.fabricOptionsForm.find(
        (item) => item.fabric == this.form.fabric
      );
      if (fabric) {
        this.maxRaster = fabric.raster;
        // console.log("调用后的maxraster", this.maxRaster, typeof this.maxRaster);
        // this.$refs.form.validateField("raster");
      }
    },
    // //打开对应图像表
    // handleEditTable(row) {
